...
const ListingStrip: React.FC<{
  ...
  useEffect(() => {
    ...
    const timer = window.setInterval(
      () => setIndex((value) => (value + 1) % listings.length),
  ...
  return (
    <section className="space-y-2">
      <div className="flex items-center justify-between">
        ...
        <button onClick={onSeeAll} className="text-xs font-black text-[#1e3a8a] flex items-center gap-1">
      ...
      {item ? (
        <div className="rounded-2xl overflow-hidden bg-white border border-slate-200 shadow-sm">
          <button onClick={() => onOpen(item)} className="relative w-full aspect-video text-left bg-slate-900">
            ...
            <span className={`absolute top-3 left-3 ${accent} text-white px-2.5 py-1 rounded-full text-[10px] font-black`}>
              {item.listingType === ListingType.RENT ? 'À LOUER' : 'À VENDRE'}
            </span>
            <div className="absolute inset-0 flex items-center justify-center">
              <span className="w-11 h-11 rounded-full bg-white/90 text-[#1e3a8a] flex items-center justify-center">
          ...
          <div className={`grid ${showCart ? 'grid-cols-3' : 'grid-cols-2'}`}>
            ...
            <button onClick={() => onChat(item)} className="py-3 text-xs font-bold text-[#16a34a] border-l border-slate-100 flex items-center justify-center gap-1">
              <MessageCircle className="w-4 h-4" /> Discuter
            </button>
            {showCart && (
              <button onClick={() => onCart?.(item)} className="py-3 text-xs font-bold text-slate-700 border-l border-slate-100 flex items-center justify-center gap-1">
        ...
        <div className="aspect-video bg-white border border-dashed border-slate-300 rounded-2xl flex items-center justify-center text-sm text-slate-500">
...
const GenericStrip: React.FC<{
  title: string;
  items: ShowcaseItem[];
  seconds: number;
  onOpen: () => void;
  accent: string;
}> = ({ title, items, seconds, onOpen, accent }) => {
  const [index, setIndex] = useState(0);
  ...
  return (
    <section className="space-y-2">
      <div className="flex items-center justify-between">
        <h3 className="font-black text-base text-slate-900 uppercase">{title}</h3>
        <button onClick={onOpen} className="text-xs font-black text-[#1e3a8a] flex items-center gap-1">
          VOIR TOUT <ArrowRight className="w-3.5 h-3.5" />
        </button>
      </div>
      <button onClick={onOpen} className="relative w-full aspect-video rounded-2xl overflow-hidden text-left bg-slate-900">
        ...
        <div className="absolute left-4 right-4 bottom-4 text-white">
          ...
          <div className="text-xs mt-1 text-white/90">{item.subtitle}</div>
        </div>
      </button>
    </section>
  );
...
export const MarketplaceView: React.FC = () => {
  ...
  useEffect(
    () => onSnapshot(doc(db, 'appSettings', 'homepage'), (snapshot) => {
  ...
  useEffect(
    () => onSnapshot(doc(db, 'appSettings', 'branding'), (snapshot) => {
  ...
  const filteredResults = useMemo(
    () => sortedListings.filter((listing) => {
      ...
      const accepted: Record<string, PropertyType[]> = {
        LAND: [PropertyType.LAND],
        APARTMENT: [PropertyType.APARTMENT],
        HOME: [PropertyType.HOUSE, PropertyType.VILLA, PropertyType.BUILDING],
      ...
      const searchableLocation = `${listing.location.address} ${listing.location.city} ${listing.location.neighborhood} ${listing.location.country}`.toLowerCase();
      if (location.trim() && !searchableLocation.includes(location.toLowerCase())) return false;

      const searchableDetails = `${listing.features.join(' ')} ${listing.bedrooms} chambres ${listing.shortDescription}`.toLowerCase();
      if (details.trim() && !searchableDetails.includes(details.toLowerCase())) return false;
  ...
  return (
    <div className="space-y-5 pb-24 max-w-4xl mx-auto">
      <section className="bg-white border border-slate-200 rounded-3xl overflow-hidden shadow-sm">
          ...
          <div className="mt-3 space-y-0.5">
            {categories.map(([value, label]) => (
              <button key={value} onClick={() => chooseCategory(value)} className="w-full flex items-center gap-2.5 py-1.5 text-left font-bold text-slate-700 min-h-9">
                <span className="w-[18px] h-[18px] rounded-full bg-[#16a34a] flex items-center justify-center shrink-0">
        ...
        <button onClick={() => document.getElementById('home-search')?.scrollIntoView({ behavior: 'smooth' })} className="block w-full">
          <img src={heroImage} alt="Présentation ImmoSecureNet" className="w-full aspect-[16/8] object-cover" />
        </button>
      </section>
      ...
      <section id="home-search" className="bg-white border border-slate-200 rounded-3xl overflow-hidden shadow-sm scroll-mt-24">
        <div className="grid grid-cols-2">
          <button onClick={() => setIntent('RENT')} className={`py-4 font-black ${intent === 'RENT' ? 'bg-[#16a34a] text-white' : 'bg-emerald-50 text-emerald-800'}`}>À LOUER</button>
          <button onClick={() => setIntent('SALE')} className={`py-4 font-black ${intent === 'SALE' ? 'bg-[#1e3a8a] text-white' : 'bg-blue-50 text-blue-900'}`}>À VENDRE</button>
        </div>
        ...
        <form onSubmit={search} className="p-4 space-y-3">
          <div className="relative">
            ...
            <select value={propertyType} onChange={(event) => setPropertyType(event.target.value)} className="w-full border border-slate-300 rounded-xl pl-9 pr-9 py-3 text-sm bg-white appearance-none">
          ...
          <div className="relative">
            ...
            <input value={location} onChange={(event) => setLocation(event.target.value)} placeholder="Localisation" className="w-full border border-slate-300 rounded-xl pl-9 pr-3 py-3 text-sm" />
          </div>

          <div className="relative">
            <SlidersHorizontal className="w-4 h-4 absolute left-3 top-1/2 -translate-y-1/2 text-slate-500" />
            <input value={details} onChange={(event) => setDetails(event.target.value)} placeholder="Plus de détails" className="w-full border border-slate-300 rounded-xl pl-9 pr-3 py-3 text-sm" />
          </div>

          <div className="grid grid-cols-2 gap-3">
            <div className="relative">
              <DollarSign className="w-4 h-4 absolute left-3 top-1/2 -translate-y-1/2 text-slate-500" />
              <input type="number" value={minPrice} onChange={(event) => setMinPrice(event.target.value)} placeholder="Prix min" className="w-full border border-slate-300 rounded-xl pl-9 pr-2 py-3 text-sm" />
            </div>
            <div className="relative">
              <DollarSign className="w-4 h-4 absolute left-3 top-1/2 -translate-y-1/2 text-slate-500" />
              <input type="number" value={maxPrice} onChange={(event) => setMaxPrice(event.target.value)} placeholder="Prix max" className="w-full border border-slate-300 rounded-xl pl-9 pr-2 py-3 text-sm" />
            </div>
          </div>

          <button className={`w-full py-3.5 rounded-xl text-white font-black flex items-center justify-center gap-2 ${intent === 'RENT' ? 'bg-[#16a34a]' : 'bg-[#1e3a8a]'}`}>
      ...
      {searchSubmitted && (
        <section ref={resultRef} className="space-y-3 scroll-mt-24">
          <div>
            <h2 className="font-black text-lg">Résultats — {intent === 'RENT' ? 'À louer' : 'À vendre'}</h2>
            <p className="text-xs text-slate-500">{filteredResults.length} annonce(s)</p>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            {filteredResults.map((listing) => (
              <article key={listing.id} className="bg-white border border-slate-200 rounded-2xl overflow-hidden">
                <button onClick={() => setSelectedListing(listing)} className="w-full text-left">
                  <img src={listing.mainPhoto} alt={listing.title} className="w-full aspect-video object-cover" />
                  <div className="p-3">
                    <h3 className="font-black text-sm">{listing.title}</h3>
                    <div className="mt-2 flex justify-between text-xs">
                      <span>{listing.location.neighborhood}, {listing.location.city}</span>
                      <strong className="text-[#1e3a8a]">{formatPrice(listing.price, listing.currency)}</strong>
                    </div>
                    <div className="mt-2 flex gap-3 text-[10px] text-slate-500">
                      <span className="flex gap-1"><BedDouble className="w-3 h-3" />{listing.bedrooms}</span>
                      <span className="flex gap-1"><Maximize2 className="w-3 h-3" />{listing.surface} m²</span>
                    </div>
                  </div>
                </button>
                <div className={`grid ${listing.listingType === ListingType.SALE ? 'grid-cols-2' : 'grid-cols-1'} border-t`}>
                  <button onClick={() => chat(listing)} className="py-3 text-[#16a34a] font-bold text-xs flex items-center justify-center gap-1">
                    <MessageCircle className="w-4 h-4" /> Discuter
                  </button>
                  {listing.listingType === ListingType.SALE && (
                    <button onClick={() => void addToCart(listing)} className="py-3 text-slate-700 font-bold text-xs border-l flex items-center justify-center gap-1">
                      <ShoppingCart className="w-4 h-4" /> Panier
                    </button>
                  )}
                </div>
      ...
      <ListingStrip
        ...
        onChat={chat}
        onCart={(listing) => void addToCart(listing)}
        onSeeAll={() => seeAll('SALE')}
        accent="bg-[#1e3a8a]"
      />
      ...
      <GenericStrip
        ...
        seconds={rotationSeconds}
        onOpen={() => showToast('La rubrique Expériences sera reliée à son catalogue complet.', 'info')}
        accent="bg-[#1e3a8a]"
      />
      ...
      <GenericStrip
        ...
        items={ishopItems}
        seconds={rotationSeconds}
        onOpen={() => setActiveNavTab('furniture_marketplace')}
        accent="bg-[#16a34a]"
      />
