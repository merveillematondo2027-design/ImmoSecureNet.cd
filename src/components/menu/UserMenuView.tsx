...
import { User, Mail, Phone, ShieldCheck, Building2, CalendarCheck, ShoppingCart, Heart, Settings, LogOut, LogIn, ChevronRight, Briefcase, Home, Store, BadgeCheck, MessageCircle, FileText, BarChart3, MapPin, Camera, Package, Users, CheckCircle2 } from 'lucide-react';
...
export const UserMenuView: React.FC<Props> = ({ onOpenAuth }) => {
  const { currentUser, logout, isAuthenticated, requestProfessionalRole, getAccessRequests } = useAuth();
  const { listings, setActiveNavTab, showToast } = useProperties();
  const [showProfessional, setShowProfessional] = useState(false);
  const [selectedProfessionalType, setSelectedProfessionalType] = useState<'REAL_ESTATE' | 'OWNER' | 'SELLER' | null>(null);
  const [realEstateRole, setRealEstateRole] = useState<UserRole.AGENT | UserRole.AGENCY>(UserRole.AGENT);
  ...
  const submitProfessionalRequest = async () => {
    ...
    const role = selectedProfessionalType === 'REAL_ESTATE'
      ? realEstateRole
      : selectedProfessionalType === 'OWNER'
        ? UserRole.OWNER
        : UserRole.SELLER;

    setSubmittingRequest(true);
    const result = await requestProfessionalRole(role);
    setSubmittingRequest(false);

    if (result.success) {
      setShowProfessional(false);
      setSelectedProfessionalType(null);
      showToast('Votre demande de compte Pro / Business a été soumise pour validation.', 'success');
    } else {
      showToast(result.error || 'Demande impossible.', 'error');
    }
  };
  ...
  const profileRows = professional
    ? [
  ...
  return <div className="pb-24 max-w-3xl mx-auto space-y-4">
    <section className="bg-white border border-slate-200 rounded-3xl overflow-hidden shadow-sm"><div className="h-32 bg-gradient-to-r from-[#1e3a8a] via-blue-700 to-[#16a34a] relative"><button onClick={()=>showToast('La modification de la couverture sera disponible depuis Modifier le profil.', 'info')} className="absolute right-4 top-4 rounded-full bg-black/20 text-white p-2"><Camera className="w-4 h-4"/></button></div><div className="px-5 pb-5 -mt-11 relative"><div className="w-22 h-22 rounded-full bg-white p-1 shadow-lg"><div className="w-20 h-20 rounded-full bg-[#1e3a8a] text-white flex items-center justify-center text-2xl font-black overflow-hidden">{currentUser.avatarUrl ? <img src={currentUser.avatarUrl} alt={currentUser.fullName} className="w-full h-full object-cover"/> : currentUser.fullName?.charAt(0)?.toUpperCase()}</div></div><div className="mt-3 flex items-center gap-2 flex-wrap"><h1 className="text-xl font-black">{currentUser.companyName || currentUser.fullName}</h1><ShieldCheck className="w-5 h-5 text-emerald-600"/><span className="px-2 py-1 rounded-full bg-blue-50 text-[#1e3a8a] text-[10px] font-black">{roleLabel(currentUser.role)}</span></div>{currentUser.companyName && <p className="text-sm text-slate-600 mt-1">{currentUser.fullName}</p>}<div className="mt-4 grid gap-2 text-sm text-slate-600"><div className="flex gap-2"><Mail className="w-4 h-4"/>{currentUser.email}</div>{currentUser.phone && <div className="flex gap-2"><Phone className="w-4 h-4"/>{currentUser.phone}</div>}<div className="flex gap-2"><MapPin className="w-4 h-4"/>RDC · Profil ImmoSecureNet</div></div><div className={`grid ${standard ? 'grid-cols-2' : 'grid-cols-3'} gap-2 mt-5`}>{!standard && <div className="rounded-xl bg-slate-50 p-3 text-center"><div className="font-black text-lg">{myListings.length}</div><div className="text-[10px] text-slate-500">Publications</div></div>}<div className="rounded-xl bg-slate-50 p-3 text-center"><div className="font-black text-lg">{professional ? 'PRO' : 'STD'}</div><div className="text-[10px] text-slate-500">Profil</div></div><div className="rounded-xl bg-slate-50 p-3 text-center"><BadgeCheck className="w-5 h-5 mx-auto text-emerald-600"/><div className="text-[10px] text-slate-500 mt-1">Statut</div></div></div></div></section>
    ...
    {standard && <section className="bg-white border border-blue-200 rounded-3xl p-5 shadow-sm">
      <div className="flex items-start gap-3"><div className="w-11 h-11 rounded-xl bg-blue-50 text-[#1e3a8a] flex items-center justify-center"><Briefcase className="w-5 h-5"/></div><div className="flex-1"><h2 className="font-black text-slate-900">Créer votre compte Pro / Business</h2><p className="text-xs text-slate-500 mt-1">Choisissez le profil professionnel correspondant à votre activité puis soumettez votre demande pour validation.</p></div></div>

      {pendingRequest ? <div className="mt-4 rounded-2xl bg-amber-50 border border-amber-200 p-4 flex items-start gap-3"><CheckCircle2 className="w-5 h-5 text-amber-600 shrink-0 mt-0.5"/><div><div className="font-black text-sm text-amber-900">Demande en cours de validation</div><div className="text-xs text-amber-700 mt-1">Type demandé : {roleLabel(pendingRequest.requestedRole)}. Vous serez informé après traitement par l’administration.</div></div></div> : <>
        <button onClick={()=>setShowProfessional(!showProfessional)} className="mt-4 w-full py-3 rounded-xl bg-[#1e3a8a] text-white text-sm font-bold">{showProfessional ? 'Fermer le choix' : 'Créer votre compte Pro / Business'}</button>
        {showProfessional && <div className="mt-4 space-y-4">
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
            <button onClick={()=>setSelectedProfessionalType('REAL_ESTATE')} className={`p-4 rounded-2xl border text-left transition ${selectedProfessionalType === 'REAL_ESTATE' ? 'border-[#1e3a8a] bg-blue-50 ring-2 ring-blue-100' : 'border-slate-200 hover:border-[#1e3a8a]'}`}><Briefcase className="w-5 h-5 text-[#1e3a8a] mb-2"/><div className="font-black text-sm">Agent / Agence</div><div className="text-[11px] text-slate-500 mt-1">Publier et gérer des offres immobilières pour des clients.</div></button>
            <button onClick={()=>setSelectedProfessionalType('OWNER')} className={`p-4 rounded-2xl border text-left transition ${selectedProfessionalType === 'OWNER' ? 'border-[#16a34a] bg-emerald-50 ring-2 ring-emerald-100' : 'border-slate-200 hover:border-[#16a34a]'}`}><Home className="w-5 h-5 text-[#16a34a] mb-2"/><div className="font-black text-sm">Bailleur / Propriétaire</div><div className="text-[11px] text-slate-500 mt-1">Enregistrer et publier vos propres biens immobiliers.</div></button>
            <button onClick={()=>setSelectedProfessionalType('SELLER')} className={`p-4 rounded-2xl border text-left transition ${selectedProfessionalType === 'SELLER' ? 'border-amber-500 bg-amber-50 ring-2 ring-amber-100' : 'border-slate-200 hover:border-amber-500'}`}><Store className="w-5 h-5 text-amber-600 mb-2"/><div className="font-black text-sm">Vendeur / Magasin</div><div className="text-[11px] text-slate-500 mt-1">Créer un magasin I-SHOP et publier des produits.</div></button>
          </div>

          {selectedProfessionalType === 'REAL_ESTATE' && <div className="rounded-2xl border border-slate-200 p-4"><div className="font-black text-sm mb-3">Vous êtes :</div><div className="grid grid-cols-2 gap-2"><button onClick={()=>setRealEstateRole(UserRole.AGENT)} className={`py-3 rounded-xl border text-sm font-bold ${realEstateRole === UserRole.AGENT ? 'bg-[#1e3a8a] text-white border-[#1e3a8a]' : 'bg-white text-slate-700 border-slate-300'}`}>Agent immobilier</button><button onClick={()=>setRealEstateRole(UserRole.AGENCY)} className={`py-3 rounded-xl border text-sm font-bold ${realEstateRole === UserRole.AGENCY ? 'bg-[#1e3a8a] text-white border-[#1e3a8a]' : 'bg-white text-slate-700 border-slate-300'}`}>Agence immobilière</button></div></div>}

          {selectedProfessionalType && <div className="rounded-2xl bg-slate-50 border border-slate-200 p-4"><div className="text-xs text-slate-600">Profil sélectionné</div><div className="font-black text-sm mt-1">{selectedProfessionalType === 'REAL_ESTATE' ? roleLabel(realEstateRole) : selectedProfessionalType === 'OWNER' ? 'Bailleur / Propriétaire' : 'Vendeur / Magasin I-SHOP'}</div><button disabled={submittingRequest} onClick={submitProfessionalRequest} className="mt-4 w-full py-3 rounded-xl bg-[#16a34a] text-white text-sm font-black disabled:opacity-60">{submittingRequest ? 'Envoi en cours...' : 'Soumettre ma demande'}</button><p className="text-[11px] text-slate-500 mt-2 text-center">Votre compte standard reste actif pendant l’examen de la demande.</p></div>}
        </div>}
      </>}
    </section>}
    ...
    <section className="bg-white border border-slate-200 rounded-2xl p-4 flex items-center gap-3"><BarChart3 className="w-5 h-5 text-[#1e3a8a]"/><div><div className="font-black text-sm">Activité du compte</div><div className="text-xs text-slate-500">Favoris, messages, rendez-vous et demandes sont centralisés ici.</div></div></section>
