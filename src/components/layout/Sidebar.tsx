...
export const Sidebar: React.FC = () => {
  ...
  const openComingSection = (label: string) => {
    setActiveNavTab('menu');
    showToast(`${label} : cet espace personnel sera relié à vos données.`, 'info');
  };
  ...
  return (
    <aside
      className={`fixed inset-0 z-50 md:relative md:flex flex-col w-[86vw] max-w-sm md:w-64 lg:w-72 bg-white border-r border-slate-200 shrink-0 min-h-[calc(100vh-4rem)] text-slate-800 shadow-xl md:shadow-xs transition-transform transform ${
    ...
    >
      <div className="md:hidden flex items-center justify-between px-5 py-4 border-b border-slate-200">
        ...
        <button
          onClick={goHome}
          className="w-9 h-9 rounded-full bg-slate-100 text-slate-600 flex items-center justify-center text-xl font-bold"
        ...
        {navItems.map((item) => {
          ...
          return (
            <button
              ...
              type="button"
              onClick={item.action}
              className={`w-full flex items-center gap-4 px-5 py-4 text-left border-b border-slate-100 transition-colors ${
                isActive ? 'bg-blue-50 text-[#1e3a8a]' : 'text-slate-700 hover:bg-slate-50'
              }`}
            >
