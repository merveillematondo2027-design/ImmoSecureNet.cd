...
import { Home, Search, ChevronDown, User, Heart, MessageSquare, Plus, Building2, HardHat, Sofa, Lightbulb, Shield, BookOpen, FileText, CalendarCheck, FileSignature, Image as ImageIcon } from 'lucide-react';
...
export const Sidebar: React.FC = () => {
  ...
  const navItems = [
    ...
    { id: 'search', label: 'RECHERCHER', icon: Search },
    {
      ...
      subItems: ['Location', 'Vente', 'Achat', 'Maisons', 'Appartements', 'Terrains', 'Immeubles', 'Bureaux', 'Commerces', 'Mobiliers']
    },
    {
    ...
    ...(currentUser?.role === UserRole.ADMIN
      ? [{ id: 'admin_branding', label: 'LOGO & IDENTITÉ VISUELLE', icon: ImageIcon }]
      : []),
  ];
          ...
          return (
            <div key={item.id} className="border-b border-slate-100 last:border-0">
              <button
                ...
                className={`w-full flex items-center justify-between px-4 py-3 text-left transition-colors ${isActive ? 'text-[#1e3a8a] font-bold bg-blue-50/50' : 'text-slate-700 font-semibold hover:bg-slate-50'}`}
              >
                <div className="flex items-center gap-3">
                  <item.icon className={`w-4 h-4 ${isActive ? 'text-[#1e3a8a]' : 'text-slate-400'}`} />
